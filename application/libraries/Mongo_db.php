...
//		die("modarr");
        ...
        $this->CI->load->config('database_mongo');
	...
	*/
	...
	private function connect()
	{
		...
		try
		{
			...
			$this->connect = new MongoClient($dns, $options,array("socketTimeoutMS" => "90000"));
...
$option = [];
	 	try
	 	{
	 		$documents = $this->db->{$collection}->aggregate($operation,$option);
	 		$this->_clear();
	 		if ($this->return_as == 'object')
			{
				return (object)$documents;
