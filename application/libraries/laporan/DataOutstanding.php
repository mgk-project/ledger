...
class DataOutstanding
{
    ...
    /*---z_rekening_transaksi_sales_cache---*/
    public function getSaldoSales($get_date1, $get_date2)
    {
        $seller_id = isset($this->seller_id) ? array("seller_id" => $this->seller_id) : array();
        $master_id = isset($this->master_id) ? array("master_id" => $this->master_id) : array();
        $transaksi_id = isset($this->master_id) ? array("id" => $this->master_id) : array();
        ...
        $this->ci->db->where($condites + $condites_main);
        $this->ci->db->order_by("id", "asc");
        $tbl = "z_rekening_transaksi_sales_cache";
        // $srcs_0 = $ps->fetchMovement(true);
    ...
    /*---z_sales_salesman_cache---*/
    public function getSaldoSeller($get_date1, $get_date2)
    {
        $seller_id = isset($this->seller_id) ? array("seller_id" => $this->seller_id) : array();
        $master_id = isset($this->master_id) ? array("master_id" => $this->master_id) : array();
        $transaksi_id = isset($this->master_id) ? array("id" => $this->master_id) : array();
        ...
        /*nganbil data yg batal*/
        $tbl_batal = "z_sales_batal_salesman_cache";
        //        $condites = array(
        //            // "month(dtime)" => $req_bulan_ini,
        ////            "year(dtime)" => $req_tahun,
        //            "month(dtime)" => $req_bulan_ini,
        ...
        $this->ci->db->where($condites + $condites_main);
        $this->ci->db->order_by("id", "asc");
        $srcs_000 = $this->ci->db->get($tbl_batal)->result();
        //        showLast_query("kuning");
        //        cekMerah(sizeof($srcs_000));
        // arrPrintPink($condites);
        // arrPrintPink($srcs_000);
        $src_bln_batal_now = array();
        $src_mtd_batal = array();
        foreach ($srcs_000 as $item) {
            $dtime = $item->dtime;
            $year = formatTanggal($dtime, 'Y');
            // $bulan = formatTanggal($dtime, 'm');
            $yearBln = formatTanggal($dtime, 'Y-m');

            $batal_nilai = $item->batal_nilai;

            $src_bln_batal_now[$yearBln][] = (array)$item;
            $src_mtd_batal[] = (array)$item;
        }


        /*nganbil data yg return*/
        $tbl_batal = "z_sales_return_salesman_cache";
        //        $condites = array(
        //            // "month(dtime)" => $req_bulan_ini,
        ////            "year(dtime)" => $req_tahun,
        //            "month(dtime)" => $req_bulan_ini,
        ...
        $this->ci->db->where($condites + $condites_main);
        $this->ci->db->order_by("id", "asc");
        $srcs_0000 = $this->ci->db->get($tbl_batal)->result();
        showLast_query("kuning");
        //        cekMerah(sizeof($srcs_0000));
        // arrPrintPink($condites);
        // arrPrintPink($srcs_0000);
        $src_bln_return_now = array();
        $src_mtd_return = array();
        foreach ($srcs_0000 as $item) {
            $dtime = $item->dtime;
            $year = formatTanggal($dtime, 'Y');
            // $bulan = formatTanggal($dtime, 'm');
            $yearBln = formatTanggal($dtime, 'Y-m');
            ...
            $src_bln_return_now[$yearBln][] = (array)$item;
            $src_mtd_return[] = (array)$item;
        }
        ...
        * -------------------------------------------------------*/
        ...
        // arrPrintKuning($src_ytd);
        $datas = array();
        $datas['mtd_return'] = $src_mtd_return;
        $datas['mtd_batal'] = $src_mtd_batal;
        $datas['mtd'] = $src_mtd;
        $datas['ytd'] = $src_ytd;
        $datas['ytd_pluss'] = $src_ytd_pluss;
        $datas['ytd_previous'] = $src_yang_lalu;
    ...
    public function getSaldoReturnSeller($get_date1, $get_date2)
    {
        $seller_id = isset($this->seller_id) ? array("seller_id" => $this->seller_id) : array();
        $master_id = isset($this->master_id) ? array("master_id" => $this->master_id) : array();
        $transaksi_id = isset($this->master_id) ? array("id" => $this->master_id) : array();
    ...
    public function getSaldoSellerTransaksi($get_date1, $get_date2)
    {
        $seller_id = isset($this->seller_id) ? array("seller_id" => $this->seller_id) : array();
        $master_id = isset($this->master_id) ? array("master_id" => $this->master_id) : array();
        $transaksi_id = isset($this->master_id) ? array("id" => $this->master_id) : array();
        ...
        /*nganbil data yg batal*/
        $tbl_batal = "z_sales_batal_salesman_transaksi_cache";
        $condites = array(
            // "month(dtime)" => $req_bulan_ini,
            "year(dtime)" => $req_tahun,
            // "date(dtime)<=" => $get_date2,
        ...
        /*nganbil data yg return*/
        $tbl_batal = "z_sales_return_salesman_transaksi_cache";
        $condites = array(
            // "month(dtime)" => $req_bulan_ini,
            "year(dtime)" => $req_tahun,
            // "date(dtime)<=" => $get_date2,
        ...
        * -------------------------------------------------------*/
        $awal_tahun = formatTanggal($get_date1, 'Y-01-01');
        $awal_tahun = "2021-01-01";
        $akhir_tahun = formatTanggal($get_date1, 'Y-m-01');
        // $akhir_bulan_lalu = formatTanggal(previousMonth($get_date1), 'Y-m-t');
        $akhir_bulan_lalu = previousDate($get_date1);
        // cekMerah("$get_date1 $akhir_bulan_lalu " . previousDate($get_date1));
        $condite_previous = array(
    ...
    /*--layer ke 2--*/
    public function callPerSeller($date1, $date2)
    {

        $src_00 = $this->getSaldoSeller($date1, $date2);
        $src_mtd_return = $src_00['mtd_return'];
        $src_mtd_batal = $src_00['mtd_batal'];
        $src_mtd = $src_00['mtd'];
        ...
        // foreach dari tranaksi yang dibatalkan
        $sumSubjekBatal = array();
        foreach ($src_mtd_batal as $item) {
            $subjek_id = $item['seller_id'];
            ...
            $rekening = $item['rekening'];
            $sumSubjekBatal[$subjek_id]['rekening'] = $rekening;
            foreach ($src_koloms as $src_kolom) {
                $$src_kolom = $item[$src_kolom];

                $sumSubjekBatal[$subjek_id]["now_" . $src_kolom . "_$rekening"] = $item[$src_kolom] * 1;
                //                $sumSubjek[$subjek_id]["now_" . $src_kolom . "_$rekening"] = $item[$src_kolom] * 1;
            }
            // cekBiru("$rekening");
            //             cekBiru($sumSubjekBatal);
        ...
        // foreach dari tranaksi yang direturn
        $sumSubjekReturn = array();
        foreach ($src_mtd_return as $item) {
            $subjek_id = $item['seller_id'];
            $transaksi_id = $item['transaksi_id'];
            ...
            $seller_nama = $item['seller_nama'];
            $rekening = $item['rekening'];
            $sumSubjekReturn[$subjek_id]['rekening'] = $rekening;
            foreach ($src_koloms as $src_kolom) {
                $$src_kolom = $item[$src_kolom];

                $sumSubjekReturn[$subjek_id]["now_" . $src_kolom . "_$rekening"] = $item[$src_kolom] * 1;
            }
            // cekBiru("$rekening");
            //             cekBiru($sumSubjekBatal);
        ...
        foreach ($sumSubjek as $sbj_id => $sbjDatas) {
            $sbjBatal = isset($sumSubjekBatal[$sbj_id]) ? $sumSubjekBatal[$sbj_id] : array();
            $sbjReturn = isset($sumSubjekReturn[$sbj_id]) ? $sumSubjekReturn[$sbj_id] : array();
            //            arrPrint($sbjBatal);
            if (sizeof($sbjBatal) > 0) {
                $sbjDatas = $sbjDatas + $sbjBatal;
                $sumSubjek[$sbj_id] = $sbjDatas;
            }
            if (sizeof($sbjReturn) > 0) {
                $sbjDatas = $sbjDatas + $sbjReturn;
                $sumSubjek[$sbj_id] = $sbjDatas;
            }
            //if($sbj_id == 69){
            //    arrPrintKuning($sbjDatas);
            //}
            $now_return_nilai_982 = isset($sbjDatas['now_saldo_kirim_982']) ? $sbjDatas['now_saldo_kirim_982'] : "0";
            $now_batal_nilai_9912 = isset($sbjDatas['now_batal_nilai_9912']) ? $sbjDatas['now_batal_nilai_9912'] : "0";
            ...
            $now_saldo_order_all = $now_saldo_order_582so + $now_saldo_order_588so + $now_saldo_order_382so;
            //            $now_saldo_order_netto_all = $now_saldo_order_all - $now_batal_nilai_9912 - $now_return_nilai_982;
            ...
            // $now_saldo_reject_all = $now_saldo_reject_582spd + $now_saldo_reject_382spd;
            ...
            $now_saldo_kirim_all = $now_saldo_kirim_582spd + $now_saldo_kirim_7499 + $now_saldo_kirim_382spd;
            $now_saldo_kirim_netto_all = $now_saldo_kirim_all - $now_batal_nilai_9912 - $now_return_nilai_982;
            ...
            /*
             * order untuk projek tidak ikut jadinya ditambahakan lagi di sini $now_saldo_reject_all + $now_saldo_reject_7499
             * --------------------------------------------------------*/
            $now_saldo_order_netto_all = $now_saldo_order_all - $now_saldo_reject_all - $now_saldo_closed_all - $now_batal_nilai_9912 - $now_return_nilai_982 + $now_saldo_reject_7499;
            ...
            $sumSubjek[$sbj_id]["now_saldo_batal_kirim_all"] = $now_batal_nilai_9912;
            $sumSubjek[$sbj_id]["now_saldo_return_kirim_all"] = $now_return_nilai_982;
            $sumSubjek[$sbj_id]["now_saldo_kirim_netto_all"] = $now_saldo_kirim_netto_all;
        }
    ...
    public function callPerTransaksi($date1, $date2)
    {
        $src_00 = $this->getSaldoSellerTransaksi($date1, $date2);
        ...
        foreach ($sumSubjek as $sbj_id => $sbjDatas) {
            ...
            $sumSubjek[$sbj_id]["now_saldo_batal_kirim_all"] = $now_batal_nilai_9912;
            $sumSubjek[$sbj_id]["now_saldo_return_kirim_all"] = $now_return_nilai_982;
            $sumSubjek[$sbj_id]["now_saldo_order_return_all"] = $now_return_nilai_982;
            $sumSubjek[$sbj_id]["now_saldo_order_netto_all"] = $now_saldo_order_netto_all;
            $sumSubjek[$sbj_id]["now_saldo_kirim_netto_all"] = $now_saldo_kirim_netto_all;
        ...
        $hasilOlahan_1 = array();
        foreach ($arrSubjek as $subj_id => $itemParam) {
            //            $sumParamsBatal = isset($sumSubjekBatal[$subj_id]) ? $sumSubjekBatal[$subj_id] : array();
            $sumParams = isset($sumSubjek[$subj_id]) ? $sumSubjek[$subj_id] : array();
            $transParams = isset($tr_datas[$subj_id]) ? $tr_datas[$subj_id] : array();
            $now_date = isset($arrDateNow[$subj_id]) ? $arrDateNow[$subj_id] : array();
            //arrPrint($itemParam);
            //arrPrint($sumParamsBatal);
            //            if($subj_id == 179026){
            //                arrPrint($sumParams);
            //            }
            // $hasilOlahan_1[] = $sumSubjek[$subj_id] + $outstandingSubjek[$subj_id] + $sub_outstanding;
    ...
    public function getSaldoSellerTahun($get_date1, $get_date2)
    {
        ...
        /*nganbil data yg return*/
        $tbl_return = "z_sales_return_salesman_cache";
        $condites = array(
            // "month(dtime)" => $req_bulan_ini,
            "year(dtime)" => $req_tahun,
            // "date(dtime)<=" => $get_date2,
        ...
        // $srcs_0 = $ps->fetchMovement(true);
        $srcs_0000 = $this->ci->db->get($tbl_return)->result();
        // showLast_query("kuning");
        // cekMerah(sizeof($srcs_0000));
        // arrPrintPink($condites);
        // arrPrintPink($srcs_0000);
        ...
        foreach ($srcs_0000 as $item) {
            ...
            $return_nilai = $item->saldo_kirim;

            $src_bln_return_now[$yearBln][] = (array)$item;
            $src_mtd_return[] = (array)$item;
        }

        // arrPrintHijau($src_bln_batal_now);
        /* -------------------------------------------------------
        * data awal tahun sampai akhir bulan lalu
        * -------------------------------------------------------*/
        $awal_tahun = formatTanggal($get_date1, 'Y-01-01');
    ...
    public function getSaldoReturnSellerTahun($get_date1, $get_date2)
    {
        $seller_id = isset($this->seller_id) ? array("seller_id" => $this->seller_id) : array();
        $master_id = isset($this->master_id) ? array("master_id" => $this->master_id) : array();
        $transaksi_id = isset($this->master_id) ? array("id" => $this->master_id) : array();
    ...
    public function callPerSellerTahun($date1, $date2)
    {
        ...
        // arrPrint($src_00);
        // matiHere(__LINE__ . __FILE__);
        // $src_000 = $this->getSaldoReturnSellerTahun($date1, $date2);
        ...
        foreach ($sumSubjek as $sbj_id => $sbjDatas) {
            ...
            $now_saldo_kirim_382spd = isset($sbjDatas['now_saldo_kirim_382spd']) ? $sbjDatas['now_saldo_kirim_382spd'] : "0";
            $now_batal_nilai_9912 = isset($sbjDatas['now_batal_nilai_9912']) ? $sbjDatas['now_batal_nilai_9912'] : "0";
            $now_saldo_kirim_982 = isset($sbjDatas['now_saldo_kirim_982']) ? $sbjDatas['now_saldo_kirim_982'] : "0";

            // $now_saldo_kirim_all = $now_saldo_kirim_582spd + $now_saldo_kirim_7499 + $now_saldo_kirim_382spd - $now_batal_nilai_9912 - $now_saldo_kirim_982;
            $now_saldo_batal_nretutn = $now_batal_nilai_9912 + $now_saldo_kirim_982;
            $now_saldo_kirim_all = $now_saldo_kirim_582spd + $now_saldo_kirim_7499 + $now_saldo_kirim_382spd;
            $now_saldo_kirim_all_netto = $now_saldo_kirim_all - $now_saldo_batal_nretutn;
            ...
            $now_saldo_order_netto_all = $now_saldo_order_all - $now_saldo_reject_all - $now_saldo_closed_all - $now_saldo_batal_nretutn + $now_saldo_reject_7499;
            // $now_saldo_order_netto_all = $now_saldo_order_all;

            $sumSubjek[$sbj_id]["prev_kredit_all"] = $prev_kredit_all;
            $sumSubjek[$sbj_id]["now_saldo_order_all"] = $now_saldo_order_all;
            $sumSubjek[$sbj_id]["now_saldo_kirim_all"] = $now_saldo_kirim_all;
            $sumSubjek[$sbj_id]["now_saldo_kirim_all_netto"] = $now_saldo_kirim_all_netto;
            $sumSubjek[$sbj_id]["now_kredit_all"] = $now_kredit_all;
            $sumSubjek[$sbj_id]["now_saldo_reject_all"] = $now_saldo_reject_all;
            $sumSubjek[$sbj_id]["now_saldo_closed_all"] = $now_saldo_closed_all;
            $sumSubjek[$sbj_id]["now_saldo_order_netto_all"] = $now_saldo_order_netto_all;
            ...
            $last_kredit_7499 = $prev_kredit_7499 + $now_kredit_7499;
            // $last_outstanding = $prev_kredit_all + $now_saldo_order_all - $now_saldo_closed_582spd - $now_saldo_reject_582spd - $now_saldo_kirim_all;
            $last_outstanding = $prev_kredit_all + $now_saldo_order_all - $now_saldo_closed_582spd - $now_saldo_reject_582spd - $now_saldo_kirim_all_netto;
            if ($last_outstanding < 0) {
                $sumSubjek[$sbj_id]["prev_kredit_all"] = $prev_kredit_all + ($last_outstanding * -1);
            }
            $sumSubjek[$sbj_id]["last_kredit_582spd"] = $last_kredit_582spd;
            $sumSubjek[$sbj_id]["last_kredit_allspd"] = $last_kredit_7499 + $last_kredit_582spd;
            // $sumSubjek[$sbj_id]["last_kredit_all"] = $prev_kredit_all + $now_saldo_order_all - $now_saldo_closed_582spd - $now_saldo_reject_582spd - $now_saldo_kirim_all;
            $sumSubjek[$sbj_id]["last_kredit_all"] = $last_outstanding < 0 ? 0 : $last_outstanding;
        }
