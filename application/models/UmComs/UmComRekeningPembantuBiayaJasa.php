...
class UmComRekeningPembantuBiayaJasa extends MdlMother
{
                ...
                if (sizeof($this->inParams['loop']) > 0) {
                    foreach ($this->periode as $periode) {
                        ...
                        foreach ($this->inParams['loop'] as $key => $value) {
                            ...
                            $_preValues = $this->cekPreValue($key, $this->inParams['static']['toko_id'], $periode, $this->inParams['static']['extern_id'], $date, $this->inParams['static']['cabang_id']);
...
// cekHitam(":: run ke $lCounter ::");
    ...
    private function cekPreValue($rek, $toko_id, $periode, $rekening_id, $date = "", $cabang_id)
    {
        $tgl = isset($date['tgl']) ? $date['tgl'] : date("d");
        $bln = isset($date['bln']) ? $date['bln'] : date("m");
        $thn = isset($date['thn']) ? $date['thn'] : date("Y");
