...
class UmComRekeningPembantuDepresiasi extends MdlMother
{
...
//                        $this->tableName = $table[$key]["cache"];
                        ...
                        $_preValues = $this->cekPreValue($key, $array_params['static']['toko_id'], $periode, $array_params['static']['extern_id'], $array_params['static']['gudang_id']);
...
//                                cekKuning($insertIDs);
    ...
    private function cekPreValue($rek, $toko_id, $periode, $produk_id, $gudang_id)
    {
        $tgl = date("d");
        $bln = date("m");
        $thn = date("Y");
...
//        }
    ...
    public function lookupLastEntries($toko_id)
    {
//        $periode = $this->getPeriode()['3'];
        $condite = array("trash" => "0", "toko_id" => "$toko_id");
