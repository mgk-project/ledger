...
class UmComRekeningPembantuPph extends MdlMother
{
                    ...
                    if (sizeof($this->inParams['loop']) > 0) {
                        foreach ($this->periode as $periode) {
                            ...
                            foreach ($this->inParams['loop'] as $key => $value) {
                                ...
                                $_preValues = $this->cekPreValue($key, $this->inParams['static']['toko_id'], $periode, $this->inParams['static']['extern_id'], $this->inParams['static']['cabang_id']);
...
//        mati_disini(":: --- ::");
    ...
    private function cekPreValue($rek, $toko_id, $periode, $rekening_id, $cabang_id)
    {
        $tgl = date("d");
        $bln = date("m");
        $thn = date("Y");
