...
class ComRekeningPembantuBudgetProjectSpk extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_budget_project_spk_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_budget_project_spk",
...
//                    $this->tableName = $table[$key]["cache"];
                        ...
                        $extern2ID = (isset($this->inParams['static']['extern2_id']) && ($this->inParams['static']['extern2_id'] > 0)) ? $this->inParams['static']['extern2_id'] : mati_disini(("unknown extern2ID*. Please contact system administrator."));
//                        $extern3ID = (isset($this->inParams['static']['extern3_id']) && ($this->inParams['static']['extern2_id'] > 0)) ? $this->inParams['static']['extern3_id'] : mati_disini(("unknown extern3ID*. Please contact system administrator."));
                        ...
                        $_preValues = $this->cekPreValue(
                            ...
                            $externID,
                            $extern2ID,
                            $this->inParams['static']['gudang_id'],
                            $this->inParams['static']['gudang2_id'],
                            $this->inParams['static']['gudang_wo']
//                            $extern3ID
                        );
                        ...
                        switch ($this->inParams['static']["mode_cache"]){
                            case "debet":
                                ...
                                break;
                            case "realisasi":
                                if ($_preValues['cache']['realisasi_debet'] > 0) {
                                    $preNumber = detectRekByPosition($key, $_preValues['cache']['realisasi_debet'], "debet");
                                }
                                else {
                                    $preNumber = detectRekByPosition($key, $_preValues['cache']['realisasi_kredit'], "kredit");
                                }
                                $rekening = "realisasi_debet";
                                break;
                        ...
                        if ($pakai_cache == 1) {
                            switch ($afterPosition) {
                                case "kredit":
                                    ...
                                    $this->outParams[$lCounter]["cache"][$mode][$rekening] = 0;
//                                    $this->outParams[$lCounter]["cache"][$mode]["realisasi_kredit"] = abs($afterNumber);
//                                    $this->outParams[$lCounter]["cache"][$mode]["realisasi_debet"] = 0;
                                    //  endregion cache rekening umum
                                    break;
                                case "debet":
                                    //  region cache rekening umum
                                    $this->outParams[$lCounter]["cache"][$mode][$rekening] = abs($afterNumber);
                                ...
                                case "forever":
                                    switch ($afterPosition) {
                                        ...
                                        case "debet":
                                            ...
                                            $this->outParams[$lCounter]["mutasi"]["realisasi_debet_awal"] = $_preValues["cache"]["realisaisi_debet"];
                                            $this->outParams[$lCounter]["mutasi"]["realisasi_debet_akhir"] = abs($afterNumber);

                                            $this->outParams[$lCounter]["mutasi"]["kredit_awal"] = $_preValues["cache"]["kredit"];
                                            $this->outParams[$lCounter]["mutasi"]["kredit_akhir"] = 0;
                                            $this->outParams[$lCounter]["mutasi"]["realisasi_kredit_awal"] = $_preValues["cache"]["kredit"];
                                            $this->outParams[$lCounter]["mutasi"]["realisasi_kredit_akhir"] = 0;
                                            //  endregion cache rekening umum
                                            break;
                                        default:
                                            $this->outParams[$lCounter]["mutasi"]["debet_awal"] = $_preValues["cache"]["debet"];
                                            $this->outParams[$lCounter]["mutasi"]["debet_akhir"] = 0;
                                            $this->outParams[$lCounter]["mutasi"]["realisasi_debet_awal"] = $_preValues["cache"]["debet"];
                                            $this->outParams[$lCounter]["mutasi"]["realisasi_debet_akhir"] = 0;
                                    ...
                                    switch ($position) {
                                        case "debet":
                                            if($this->inParams['static']["mode_cache"]=="debet"){
                                                $this->outParams[$lCounter]["mutasi"]["debet"] = abs($value);
                                                $this->outParams[$lCounter]["mutasi"]["kredit"] = 0;
                                            }
                                            else{
                                                $this->outParams[$lCounter]["mutasi"]["realisasi_debet"] = abs($value);
                                                $this->outParams[$lCounter]["mutasi"]["realisasi_kredit"] = 0;
                                            }
                                        ...
                                        case "kredit":
                                            if($this->inParams['static']["mode_cache"]=="debet"){
                                                ...
                                                $this->outParams[$lCounter]["mutasi"]["realisasi_kredit"] = abs($value);
                                                $this->outParams[$lCounter]["mutasi"]["realisasi_debet"] = 0;
                                            }
...
//        matiHEre(__LINE__);
    ...
    private function cekPreValue($rek, $produk_id,$cabang_id, $periode, $rekening_id, $rekening2_id,$gudangID,$gudang2ID,$gudangWo)
    {
        $tgl = date("d");
        $bln = date("m");
        $thn = date("Y");
        ...
        $this->addFilter("gudang_id='$gudangID'");
        $this->addFilter("gudang2_id='$gudang2ID'");
        $this->addFilter("gudang_wo='$gudangWo'");
        $result = array();
        $localFilters = array();
        ...
        if (sizeof($tmp) > 0) {
            ...
            foreach ($tmp as $row) {
                $result["cache"] = array(
                    ...
                    "realisasi_debet" => $row->realisasi_debet,
                    "realisasi_kredit" => $row->realisasi_kredit,
                    "realisasi_qty_debet" => $row->realisasi_debet,
                    "realisasi_qty_kredit" => $row->realisasi_kredit,
                    // saldo bawah
            ...
            if (!in_array($rekCat, $this->catException)) {
                ...
                if (sizeof($tmp) > 0) {
                    ...
                    $result["cache"] = array(
                        ...
                        "realisasi_debet" => 0,
                        "realisasi_kredit" => 0,
                        "realisasi_qty_debet" => 0,
                        "realisasi_qty_kredit" => 0,
                        // saldo bawah
                        "saldo_debet" => 0,
                        "saldo_kredit" => 0,
                        "saldo_debet_periode" => 0,
