...
class ComRekeningTransaksiPembantu extends MdlMother
{
...
//     private $periode = array("harian", "bulanan", "tahunan");
                ...
                foreach ($this->inParams as $array_params) {
                    ...
                    foreach ($array_params['loop'] as $key => $x) {
                        ...
                        $_preValues = $this->cekPreValue(
                            ...
                            $array_params['static']['transaksi_id'],
                        ...
                        if (array_key_exists("id", $_preValues["cache"]) && ($_preValues["cache"]["id"] > 0)) {
                            ...
                            $date_ex = explode("-", $array_params['static']['fulldate']);
                            $mode = "insert";
                            $_preValues_id = 0;
                        ...
                        if ($pakai_cache == 1) {
                            ...
                            if ($array_params['static']['transaksi_tipe'] == "rejected") {
                                //                                $this->outParams[$lCounter]["cache"][$mode]["rejected"] = $_preValues["cache"]["rejected"] + abs($unit);
                                $this->outParams[$lCounter]["cache"][$mode]["rejected"] = abs($unit);
                                $this->outParams[$lCounter]["cache"][$mode]["qty_debet_lap"] = $_preValues["cache"]["qty_debet_lap"];
                                $this->outParams[$lCounter]["cache"][$mode]["qty_kredit_lap"] = 0;
                                $this->outParams[$lCounter]["cache"][$mode]["debet_lap"] = $_preValues["cache"]["debet_lap"];
                                $this->outParams[$lCounter]["cache"][$mode]["kredit_lap"] = 0;
                            }
                            if ($array_params['static']['transaksi_tipe'] == "closed") {
                                $this->outParams[$lCounter]["cache"][$mode]["closed"] = $_preValues["cache"]["closed"] + abs($unit);
                                $this->outParams[$lCounter]["cache"][$mode]["qty_debet_lap"] = $_preValues["cache"]["qty_debet_lap"];
                                $this->outParams[$lCounter]["cache"][$mode]["qty_kredit_lap"] = $_preValues["cache"]["qty_kredit_lap"];
                                $this->outParams[$lCounter]["cache"][$mode]["debet_lap"] = $_preValues["cache"]["debet_lap"];
                                $this->outParams[$lCounter]["cache"][$mode]["kredit_lap"] = $_preValues["cache"]["kredit_lap"];
                            }
                            if ($array_params['static']['transaksi_tipe'] == "edited") {
                                $this->outParams[$lCounter]["cache"][$mode]["qty_debet_lap"] = 0;
                                $this->outParams[$lCounter]["cache"][$mode]["qty_kredit_lap"] = 0;
                                $this->outParams[$lCounter]["cache"][$mode]["debet_lap"] = 0;
                                $this->outParams[$lCounter]["cache"][$mode]["kredit_lap"] = 0;
                            }
                            if ($array_params['static']['transaksi_tipe'] == "batal") {
