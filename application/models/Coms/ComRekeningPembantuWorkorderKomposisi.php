...
class ComRekeningPembantuWorkorderKomposisi extends MdlMother
{
...
//        "tahunan",
    ...
    public function __construct()
    {
        ...
        $this->tableName = "project_komposisi_workoder";
        $this->tableName_master = array(
            "mutasi" => "project_komposisi_workoder_mutasi",
        );
    }
...
//                        }
                        ...
                        $preNumber = $_preValues["cache"]["saldo"];
                        $preQtyNumber = $_preValues["cache"]["qty_saldo"];
...
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_qty_kredit_periode"] = $_preValues["cache"]["saldo_qty_kredit_periode"] + abs($unit);
                                    $this->outParams[$lCounter]["cache"][$mode]["kredit"] = $_preValues["cache"]["kredit"] + abs($value);
                                    $this->outParams[$lCounter]["cache"][$mode]["qty_kredit"] = $_preValues["cache"]["qty_kredit"] + abs($unit);
                                    break;
                                case "debet":
...
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_qty_debet_periode"] = $_preValues["cache"]["saldo_qty_debet_periode"] + abs($unit);
                                    $this->outParams[$lCounter]["cache"][$mode]["debet"] = $_preValues["cache"]["debet"] + abs($value);
                                    $this->outParams[$lCounter]["cache"][$mode]["qty_debet"] = $_preValues["cache"]["qty_debet"] + abs($unit);
                            ...
                            $this->outParams[$lCounter]["cache"][$mode]["saldo"] = $afterNumber;
                            $this->outParams[$lCounter]["cache"][$mode]["qty_saldo"] = $afterQtyNumber;
...
//arrPrintPink($this->outParams[$lCounter]["cache"]);
                                                ...
                                                foreach ($pSpec_mode as $sub_mode => $pSpec_mode_data) {
                                                    ...
                                                    switch ($sub_mode) {
                                                        case "insert":
                                                            ...
                                                            break;
                                                        case "update":
//arrPrintHijau($pSpec_mode_data);
                                                            unset($pSpec_mode_data["jenis"]);
                                                            $this->db->where('id', $id);
...
//        mati_disini(__LINE__);
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $produk_id, $extern2_id, $extern3_id, $gudang_id, $date = NULL)
    {
        if ($date != NULL) {
...
//        $this->addFilter("periode='$periode'");
        ...
        $this->addFilter("fase_id='$extern2_id'");
        $this->addFilter("produk_dasar_id='$extern3_id'");
