...
class ComRekeningPembantuPiutangSupplierDetailTransMain extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_subpiutangsuppliertrans_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_subpiutangsuppliertrans",
        );
        ...
        if (sizeof($this->inParams['loop']) > 0) {
            ...
            foreach ($this->periode as $periode) {
                ...
                foreach ($this->inParams['loop'] as $key => $value) {
                    ...
                    $msg = "Transaksi gagal disimpan karena jenis diskon supplier tidak dikenali. Silahkan hubungi admin.";
//                    $externID = (isset($inParams['static']['extern_id']) && $inParams['static']['extern_id'] > 0) ? $inParams['static']['extern_id'] : mati_disini($msg);
//                    $externID = (isset($inParams['static']['transaksi_id']) && $inParams['static']['transaksi_id'] > 0) ? $inParams['static']['transaksi_id'] : mati_disini($msg);
                    if (isset($inParams['static']['extern_id']) && $inParams['static']['extern_id'] > 0) {
                        $externID = $inParams['static']['extern_id'];
                    }
                    else {
                        if (isset($inParams['static']['transaksi_id']) && $inParams['static']['transaksi_id'] > 0) {
                            if ($inParams['static']['jenis'] == $this->jenis_pembatalan) {
                                $cCode = "_TR_" . $this->jenis_pembatalan;
                                $externID = $_SESSION[$cCode]["main"]["referenceID"];
                            }
                            else {
                                $externID = $inParams['static']['transaksi_id'];
                            }
                        }
                        else {
                            mati_disini($msg);
                    ...
                    $extern2ID = (isset($inParams['static']['extern2_id']) && $inParams['static']['extern2_id'] > 0) ? $inParams['static']['extern2_id'] : mati_disini($msg);
                    $extern2Nama = (isset($inParams['static']['extern2_nama'])) ? $inParams['static']['extern2_nama'] : "";
                    $msg = "Transaksi gagal disimpan karena supplier tidak dikenali. Silahkan hubungi admin.";
                    $extern3ID = (isset($inParams['static']['extern3_id']) && $inParams['static']['extern3_id'] > 0) ? $inParams['static']['extern3_id'] : mati_disini($msg);
                    ...
                    // replace
                    $this->inParams['static']['extern_id'] = $externID;
                    $this->inParams['static']['extern_nama'] = $inParams['static']['transaksi_no'];
//matiHere($externID);
...
//
                        if ($afterPosition != detectRekDefaultPosition($key) && ($afterNumber != 0)) {
                            ...
                            mati_disini(("Saldo rekening $rekening_alias $extern2Nama $extern3Nama tidak cukup. Anda membutuhkan $value, tersedia $preNumber. Silahkan menghubungi admin."));
...
//                            unset($pSpec_mode["tabel"]);
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $pihak_id, $pihak2_id, $pihak3_id)
    {
        $tgl = date("d");
        $bln = date("m");
        $thn = date("Y");
