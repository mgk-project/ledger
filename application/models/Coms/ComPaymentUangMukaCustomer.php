...
class ComPaymentUangMukaCustomer extends MdlMother
{
...
//    private $periode = array("harian", "bulanan", "tahunan", "forever");
    ...
    {
        $this->tableName = "transaksi_uang_muka_source_cache";
        $this->tableName_mutasi = "transaksi_uang_muka_source_mutasi";
        $this->tableName_master = array(
            "mutasi" => "transaksi_uang_muka_source_mutasi",
        );
    }
...
//                $this->tableName_mutasi = $table[$key]["mutasi"];
                ...
                $msg = "Silahkan memilih sumber pembayaran sebelum melanjutkan transaksi.";
//                $extern_id = $externID = (isset($this->inParams['static']['extern_id']) && ($this->inParams['static']['extern_id'] > 0)) ? $this->inParams['static']['extern_id'] : die(lgShowAlert("$msg"));
//                $label = (isset($this->inParams['static']['label']) && ($this->inParams['static']['extern_id'] > 0)) ? $this->inParams['static']['label'] : "";
//                $label2 = (isset($this->inParams['static']['extern_label2']) && ($this->inParams['static']['extern_id'] > 0)) ? $this->inParams['static']['extern_label2'] : "";
//                $cabang_id = (isset($this->inParams['static']['cabang_id']) && ($this->inParams['static']['extern_id'] > 0)) ? $this->inParams['static']['cabang_id'] : "";
                $extern_id = $externID = (isset($this->inParams['static']['extern_id'])) ? $this->inParams['static']['extern_id'] : die(lgShowAlert("$msg"));
                $label = (isset($this->inParams['static']['label'])) ? $this->inParams['static']['label'] : "";
                $label2 = (isset($this->inParams['static']['extern_label2'])) ? $this->inParams['static']['extern_label2'] : "";
                ...
                if (in_array($key, $configBalanceProtections)) {
                    if ($afterPosition != detectRekDefaultPosition($key) && ($afterNumber != 0)) {
                        $afterNumber_ex = reformatExponent($afterNumber);
                        cekMErah($afterNumber_ex);
                        if ($afterNumber_ex < 100) {

                        }
                        else {
                            mati_disini("*Saldo rekening $key " . $this->inParams['static']['extern_nama'] . " tidak cukup. Silahkan menghubungi admin.");
...
//                    }
    ...
    private function cekPreValue($label, $cabang_id, $extern_id, $label2)
    {

        $this->addFilter("label='$label'");
        $this->addFilter("cabang_id='$cabang_id'");
        $this->addFilter("extern_label2='$label2'");
