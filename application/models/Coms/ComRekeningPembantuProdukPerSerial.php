...
class ComRekeningPembantuProdukPerSerial extends MdlMother
{
...
//    private $periode = array("harian", "bulanan", "tahunan", "forever");
    ...
    {
        $this->tableName = "_rek_pembantu_produk_perserial_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_produk_perserial",
        );
    }
                ...
                foreach ($this->periode as $periode) {
                    foreach ($this->inParams as $array_params) {
                        if (isset($array_params["static"]["kategori_id"]) && $array_params["static"]["kategori_id"] == "4") {
                            //skip produk jasa tidak perlu cache akrena tidak punya stok
                            $insertIDs[] = 1;
                        }
                        else {
                            arrPrintWebs($array_params);
                            $arrRekening = array();
                            foreach ($array_params['loop'] as $key => $x) {
                                ...
                                $mode = isset($array_params['static']['mode']) ? $array_params['static']['mode'] : "ada";
                                ...
                                if ($mode == "insert") {
                                    ...
                                    $_preValues = $this->cekPreValue(
                                        ...
                                        $array_params['static']['extern_nama'],
                                        $array_params['static']['extern2_nama'],
...
//                                cekLime("$afterQtyNumber = $preQtyNumber + $unit;");
//                                cekLime("$afterNumber = $preNumber + $value;");
...
//                                cekMerah("$key");
//                                arrPrint($_preValues['cache']['qty_debet']);


                                $afterNumberAvg = $afterQtyNumber == 0 ? 0 : $afterNumber / $afterQtyNumber;

                                $extern_nama_ex = explode("_", $array_params['static']['extern_nama']);

                                if ($afterQtyNumber > 1) {
                                    $pSerial = $array_params['static']['extern_name'];
                                    $pName = $array_params['static']['produk_nama'];
                                    $msg = "Serial $pSerial untuk produk $pName terdeteksi ganda. Silahkan diperiksa kembali <b> @" . __LINE__ . " " . __METHOD__ . "</b>";
                                    cekHitam("afterposision ::" . $afterPosition . " after qty ::" . $afterQtyNumber);
                                    cekBiru("$afterQtyNumber = $preQtyNumber + $unit;");
                                    mati_disini($msg);
                                }
                                elseif ($afterQtyNumber < 0) {
                                    $pSerial = $array_params['static']['extern_nama'];
                                    $pName = $array_params['static']['produk_nama'];
                                    $cName = $array_params['static']['cabang_nama'];
                                    $gName = $array_params['static']['gudang_nama'];
                                    if (isset($array_params['static']["rejection"]) && $array_params['static']["rejection"] == 1) {
                                        $msg = "Serial $pSerial untuk produk $pName tidak terdaftar [key:: $key]. Nomer serial harus terdaftar pada transaski ini, tidak boleh ditukar. Silahkan diperiksa kembali <b> @" . __LINE__ . " " . __METHOD__ . "</b>";
                                    }
                                    else {
                                        $msg = "Serial $pSerial untuk produk $pName tidak terdaftar di $cName gudang $gName [key:: $key]. Silahkan diperiksa kembali <b> @" . __LINE__ . " " . __METHOD__ . "</b>";
                                    }
                                    cekHitam("afterposision ::" . $afterPosition . " after qty ::" . $afterQtyNumber);
                                    cekBiru("$afterQtyNumber = $preQtyNumber + $unit;");
                                    mati_disini($msg);
                                }
                                cekHitam("[$afterPosition] [$afterNumber] [$afterQtyNumber]");

                                if (sizeof($extern_nama_ex) > 1) {
                                    $insertIDs[$lCounter] = $lCounter;
//                                    cekHitam("BY PASS");
                                }
                                else {
//                                    cekHitam("PROSES SERIAL [key:: $key] " . $extern_nama_ex[0]);
//                                    arrPrintKuning($configBalanceProtections);
                                    if (in_array($key, $configBalanceProtections)) {
                                        if ($afterPosition != detectRekDefaultPosition($key) && ($afterQtyNumber != 0)) {
                                            $pSerial = $array_params['static']['extern_nama'];
                                            $pName = $array_params['static']['produk_nama'];
                                            $msg = "Serial $pSerial untuk produk $pName tidak terdaftar [key:: $key]. Silahkan diperiksa kembali <b> @" . __LINE__ . " " . __METHOD__ . "</b>";
                                            cekHitam("afterposision ::" . $afterPosition . " after qty ::" . $afterQtyNumber);
                                            cekBiru("[$afterPosition] $afterQtyNumber = $preQtyNumber + $unit;");
                                            mati_disini($msg);
                                        }
                                    }
...
//                                        $this->outParams[$lCounter]["mutasi"]["harga_awal"] = abs($_preValues['cache']['harga']);
            ...
            if (sizeof($insertIDs) > 0) {
                ...
                matiHere("transaksi gagal disimpan karena sesi anda tidak valid/kadaluarsa. silahkan refresh browser atau relogin. code: " . __LINE__);
                return false;
            }
        }
        else {
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $extern_id, $extern_nama, $extern2_nama, $produk_id, $gudang_id, $date = NULL)
    {
        if ($date != NULL) {
        ...
        $this->addFilter("extern_nama='$extern_nama'");
//        $this->addFilter("extern2_nama='$extern2_nama'");
...
//            $this->addFilter("extern2_nama='$extern2_nama'");
    ...
    {//==memanggil saldo2 dari rekening tertentu
        $this->db->select("*");
//        $this->db->select("id,rek_id,debet,kredit,qty_debet,qty_kredit,produk_id,produk_nama,periode,rekening,extern_nama,gudang_id");
...
* =====================================================================*/
    public function fetchLastMoves_()
    {
        $this->load->helper("he_mass_table");
