...
class ComRugiLaba_forever extends MdlMother
{
...
//
        ...
        $cr->addFilter("thn<='" . $this->inParams['static']['tahun'] . "'");
//        $cr->addFilter("thn='" . $this->inParams['static']['tahun'] . "'");
        $tmp = $cr->fetchAllBalancesTahunan();
...
//            cekHitam(":: MENULIS RUGILABA KE TABEL");
...
//                    cekLime($arrSpec['rekening']);
                    unset($tmp[$arrSpec['rekening']]);
                }
                else{
//                    cekOrange($arrSpec['rekening']. " :: " . $arrSpec['debet']. " :: " . $arrSpec['kredit']);
...
//                    $tmp['laba ditahan']['debet'] = $tmp['laba ditahan']['debet']+$arrSpec['kredit'];
//                    $tmp['laba ditahan']['kredit'] = $tmp['laba ditahan']['kredit']+$arrSpec['debet'];
                }
            }
        }
...
//arrPrint($tmp);
        foreach ($tmp as $tmpSpec){
            ...
            $total['debet'] += $tmpSpec['debet'];
            $total['kredit'] += $tmpSpec['kredit'];
        }
//        arrPrint($total);
//        $selisih = $total['debet']-$total['kredit'];
//        cekMerah("selisih: " . $selisih);
//        mati_disini();
...
//        cekMerah($this->db->last_query());
    ...
    private function cekPreValue($rek, $cabang_id, $periode="forever")
    {
        $tgl = date("d");
        $bln = date("m");
        $thn = date("Y");
