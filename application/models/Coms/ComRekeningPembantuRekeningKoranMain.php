...
class ComRekeningPembantuRekeningKoranMain extends MdlMother
{
...
//        $this->tableName_fifoAvg = "fifo_avg";
        $this->tableName = "_rek_pembantu_rekeningkoran_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_rekeningkoran",
...
//mati_disini();
        ...
        $this->inParams = $inParams;
        $array_params = $inParams;
        if (sizeof($this->inParams) > 0) {
            $lCounter = 0;
            foreach ($this->periode as $periode) {

                    $arrRekening = array();
                    foreach ($array_params['loop'] as $key => $value_item) {
//                        $lCounter++;
...
//                        $this->tableName = $table[$key]["cache"];
                        ...
                        $extern2ID = isset($array_params['static']['extern2_id']) ? $array_params['static']['extern2_id'] : 0;

                        $_preValues = $this->cekPreValue($key, $array_params['static']['cabang_id'], $periode, $array_params['static']['extern_id'], $extern2ID);
...
cekPink("nilai rekening koran: $afterNumber, position: $afterPosition, prenumber: $preNumber");
