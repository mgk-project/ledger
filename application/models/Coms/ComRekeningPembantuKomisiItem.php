...
class ComRekeningPembantuKomisiItem extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_komisi_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_komisi",
        );
            ...
            foreach ($this->periode as $periode) {
                foreach ($this->inParams as $array_params) {
                    ...
                    foreach ($array_params['loop'] as $key => $value) {
                        ...
                        $msg = "Silahkan memilih dahulu freelancer/cabang sebelum melanjutkan transaksi.";
                        $externID = (isset($array_params['static']['extern_id']) && ($array_params['static']['extern_id'] != NULL)) ? $array_params['static']['extern_id'] : die(lgShowAlert("$msg"));
                        ...
                        if (array_key_exists("id", $_preValues["cache"]) && ($_preValues["cache"]["id"] > 0)) {
                            ...
                            $mode = "insert";
                            $_preValues_id = 0;
                            if (isset($array_params['static']['fulldate'])) {
                                $fulldate_ex = explode("-", $array_params['static']['fulldate']);
                                $tgl = $fulldate_ex[2];
                                $bln = $fulldate_ex[1];
                                $thn = $fulldate_ex[0];
                            }
