...
class ComRekeningPembantuRelasiRekeningKoran extends MdlMother
{
...
//        $this->tableName_fifoAvg = "fifo_avg";
        $this->tableName = "_rek_pembantu_bank_relasi_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_bank_relasi",
...
//mati_disini();
        ...
        if (sizeof($this->inParams) > 0) {
            ...
            foreach ($this->periode as $periode) {
                if (isset($this->inParams['loop'])) {
                    foreach ($this->inParams['loop'] as $key => $value_item) {
//                        $lCounter++;

//                        $value_item = $this->inParams['static']['produk_nilai'];
//                        $unit = $this->inParams['static']['produk_qty'];
...
//                        $this->tableName = $table[$key]["cache"];
                        ...
                        $extern2ID = isset($this->inParams['static']['extern2_id']) ? $this->inParams['static']['extern2_id'] : 0;
                        $_preValues = $this->cekPreValue($key, $this->inParams['static']['cabang_id'], $periode, $this->inParams['static']['extern_id'], $extern2ID);
...
//                        $afterQtyPosition = detectRekPosition($key, $afterQtyNumber);
                        ...
                        if (in_array($key, $configBalanceProtections)) {
                            if ($afterPosition != detectRekDefaultPosition($key)) {
...
//                                        $msg = "Transaksi gagal karena terjadi saldo unit " . $this->inParams['static']['extern_id'] . " bernilai minus (" . $this->outParams[$lCounter]['mutasi']['unit_af'] . ")";
//                                        die(lgShowAlert($msg));
//                                    }
...
//                }
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $produk_id, $produk2_id)
    {
        $tgl = date("d");
        $bln = date("m");
        $thn = date("Y");
