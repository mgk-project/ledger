...
//                    $this->tableName = $table[$key]["cache"];
                    ...
                    $unit = isset($inParams['static']['qty']) ? $inParams['static']['qty'] : 0;
...
//                        $preQty = detectRekByPosition($key, $_preValues['cache']['qty_kredit'], "kredit");
                    ...
                    if (in_array($key, $configBalanceProtections)) {
                        ...
                        if ($afterQtyNumber != detectRekDefaultPosition($key) && ($afterQtyNumber != 0)) {
                            die(lgShowAlert("insufficient balance for $key. <br>You requested $unit. <br>Available: $preQtyNumber. <br>Post-value should be: $afterQtyNumber"));
                        }
                    }
...
//                                $this->outParams[$lCounter]["cache"][$mode]["qty_kredit"] = 0;
                    ...
                    //  endregion cache rekening pembantu
                    arrPrint($this->outParams[$lCounter]["cache"]);
                    matiHEre("hooppp dulu");
