...
class ComPaymentSourceItem2 extends MdlMother
{
...
//                            $targetKey = "diskon";
                    ...
                    if ($prev['id'] > 0) {
                        if ($prev['sisa'] >= $inSpec['static']['terbayar']) {
                            ...
                            $tbl_id = $prev['id'];
                            matiHere("[$tbl_id] nilai maksimal dari transaksi hanya diperbolehkan senilai saldo uang muka = ".$prev['sisa']);
//                            $jmlBayar = $prev['sisa'];
                        }
...
//                        $newSisa = $prev['sisa'];
    ...
    private function cekPreValue($targetJenis, $transaksiID, $label, $cabangID, $externID,$extern2ID)
    {


        $this->load->model("Mdls/MdlPaymentSource");
        $tr = new MdlPaymentSource();
        $tr->setFilters(array());
