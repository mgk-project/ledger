...
class ComTransaksiKreditLimit extends MdlMother
{
    ...
    public function __construct()
    {
        ...
        $this->tableName = "z_transaksi_kreditlimit_cache";
        $this->tableName_master = array(
            "mutasi" => "z_transaksi_kreditlimit_mutasi",
        );
    }
                                                ...
                                                foreach ($pSpec_mode as $sub_mode => $pSpec_mode_data) {
                                                    ...
                                                    switch ($sub_mode) {
                                                        case "insert":
                                                            ...
                                                            cekHijau("$sub_mode :: " . $this->db->last_query());
                                                            break;
                                                        case "update":
...
//            $this->addFilter("gudang_id='$gudang_id'");
            ...
            if (sizeof($tmp) > 0) {
                ...
                $c = New MdlCustomer();
                $c->addFilter("id='$produk_id'");
                $cTmp = $c->lookupAll()->result();
                if(sizeof($cTmp)>0){
                    $result["cache"] = array(
                        "debet" => $cTmp[0]->kredit_limit,
                        "kredit" => 0,
                        "qty_debet" => 0,
                        "qty_kredit" => 0,
                        "harga" => 0,
...
//        $rek = "kas";
    ...
    public function fetchBalancesKredit()
    {
        $this->db->select("*");
