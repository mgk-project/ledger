...
class ComRekeningPembantuHppProjectBiayaLain extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_biaya_project_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_biaya_project",
    ...
    public function pair($inParams)
    {
        ...
        if(count($inParams)>0){
            foreach($inParams as $paramDatas){
                if (sizeof($paramDatas['loop']) > 0) {
                    $lCounter = 0;
                    foreach ($this->periode as $periode) {
                        $arrRekening = array();
                        foreach ($paramDatas['loop'] as $key => $value) {
                            $lCounter++;
...
//                    $this->tableName = $table[$key]["cache"];
                            ...
                            $externID = (isset($paramDatas['static']['extern_id']) && ($paramDatas['static']['extern_id'] > 0)) ? $paramDatas['static']['extern_id'] : mati_disini(("unknown projectID*. Please contact system administrator."));
//                            $extern2ID = (isset($paramDatas['static']['extern2_id']) && ($paramDatas['static']['extern2_id'] > 0)) ? $paramDatas['static']['extern2_id'] : mati_disini(("unknown customerID*. Please contact system administrator."));

                            //  region cek saldo awal
                            $_preValues = $this->cekPreValue(
                                $key,
                                $paramDatas['static']['cabang_id'],
                                $periode,
                                $externID
//                                $extern2ID
                            );
...
//                    $afterNumber = round($preNumber + $value);
                            ...
                            if (in_array($key, $configBalanceProtections)) {
                                if ($afterPosition != detectRekDefaultPosition($key) && ($afterNumber != 0)) {
                                    die(lgShowAlert("insufficient balance for $key " . $paramDatas['static']['extern_nama'] . ""));
                                }
                            }
...
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_kredit_periode"] = 0;
                                ...
                                $this->outParams[$lCounter]["cache"][$mode]["rek_id"] = createRekCode($key, $paramDatas['static']['extern_id']);
                                ...
                                foreach ($paramDatas['static'] as $key_static => $value_static) {
                            ...
                            if ($pakai_mutasi == 1) {
                                switch ($periode) {
                                    case "forever":
                                        ...
                                        foreach ($paramDatas['static'] as $key_static_mutasi => $value_static_mutasi) {
                                            if (in_array($key_static_mutasi, $this->outFieldsMutasi)) {
                                                $this->outParams[$lCounter]["mutasi"][$key_static_mutasi] = $value_static_mutasi;
                                            }
                                        }
                                        $this->outParams[$lCounter]["mutasi"]["rek_id"] = createRekCode($key, $paramDatas['static']['extern_id']);
...
//                matiHere();
            ...
            matiHere("empty data to exec detect");
        }
