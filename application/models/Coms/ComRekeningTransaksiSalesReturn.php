...
class ComRekeningTransaksiSalesReturn extends MdlMother
{
...
//        "gudang_nama",
    ...
    {
        // TOTAL KONSOLIDASIAN
        $this->tableName = "z_sales_return_cache";
        $this->tableName_master = array(
            "mutasi" => "z_sales_return_mutasi",
        );
    }
...
//    private function cekPreValue($rek, $cabang_id, $periode, $produk_id, $gudang_id, $date = NULL, $transaksi_id, $seller_id)
    private function cekPreValue($rek, $periode, $date = NULL, $transaksi_id)
    {
        if ($date != NULL) {
            $date_ex = explode("-", $date);
            $tgl = $date_ex[2];
