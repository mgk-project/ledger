...
class ComJurnal_activity extends MdlMother
{
...
//        "tahunan",
    ...
    {
        $this->tableName = "activity_master_cache";
        $this->tableName_master = array(
            "cache" => "activity_master_cache",
            "mutasi" => "activity_master_mutasi",
        ...
        if (sizeof($this->inParams['loop']) > 0) {
            ...
            foreach ($this->periode as $periode) {
                ...
                foreach ($this->inParams['loop'] as $key => $value) {
                    ...
                    $transaksiID = $inParams['static']['transaksi_id'];
                    $masterID = $inParams['static']['master_id'];
//                    $jenis = $inParams['static']['jenis'];
                    $jenis = $inParams['static']['jenis_master'];
                    $step_number = $inParams['static']['step_number'];
                    $cabangID = $inParams['static']['cabang_id'];
                    $cabang2ID = $inParams['static']['cabang2_id'];
                    $partialOtorisasi = isset($inParams['static']['partial_otorisasi']) ? $inParams['static']['partial_otorisasi'] : false;

                    $new_sisa = isset($inParams['static']['new_sisa']) ? $inParams['static']['new_sisa'] : 0;
                    if ($new_sisa > 0) {
                        $running = false;
                    }
                    else {
                    ...
                    if ($partialOtorisasi == true) {
                        $running = false;
                    }

                    cekKuning("### $new_sisa, $running ###");
                    if ($running == true) {
                        ...
                        if (isset($this->regulerRoutesConfig[$jenis][$step_number]) && sizeof($this->regulerRoutesConfig[$jenis][$step_number]) > 0) {
                            ...
                            cekMerah("max step: " . sizeof($this->regulerRoutesConfig[$jenis]));
...
//                                    $this->outParams[$lCounter]["mutasi"]["rek_id"] = createRekCode($key);
                                ...
                                if ($pakai_jurnal == 1) {
                                    foreach ($this->inParams['static'] as $key_static_jurnal => $value) {
                                        if (in_array($key_static_jurnal, $this->outFieldsJurnal)) {
