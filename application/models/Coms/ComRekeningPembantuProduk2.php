...
class ComRekeningPembantuProduk2 extends MdlMother
{
...
//        return $result->result();
    ...
    public function pair2($inParams)
    {
        $this->load->helper("he_mass_table");
        $configBalanceProtections = $this->config->item("accountBalanceProtections");
        $this->inParams = $inParams;
...
//                        cekhitam(":: $afterNumber, val: $value,  $afterPosition, afterQty: $afterQtyNumber");
                        if (in_array($key, $configBalanceProtections)) {
                            if ($afterPosition != detectRekDefaultPosition($key) && ($afterQtyNumber != 0)) {
                                $pName = $array_params['static']['extern_name'];
                                mati_disini(("Saldo tidak cukup untuk $key, $pName. Anda membutuhkan $value, tersedia $preNumber. Silahkan menghubungi admin."));
                            }
                        }
                        ...
                        if ($pakai_mutasi == 1) {
                            switch ($periode) {
                                case "forever":
                                    ...
                                    //--------------------------------------------
                                    if(!isset($totalPeriodeValue[$key])){
                                        $totalPeriodeValue[$key] = 0;
                                    }
                                    $totalPeriodeValue[$key] += $value;
                                    //--------------------------------------------
