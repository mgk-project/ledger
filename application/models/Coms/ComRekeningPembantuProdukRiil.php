...
class ComRekeningPembantuProdukRiil extends MdlMother
{
...
//                        cekhitam(":: $afterNumber, val: $value,  $afterPosition");
                        if (in_array($key, $configBalanceProtections)) {
                            if ($afterPosition != detectRekDefaultPosition($key) && ($afterQtyNumber != 0)) {
                                ...
                                $pQty = $array_params['static']['produk_qty'];
                                cekMerah("insufficient balance for* $key [$pId] [$pName] [$pQty]. You requested $value. Available: $preNumber");
