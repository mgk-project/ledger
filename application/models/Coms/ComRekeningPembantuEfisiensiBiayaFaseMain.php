...
class ComRekeningPembantuEfisiensiBiayaFaseMain extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_efisiensi_fase_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_efisiensi_fase",
        );
...
//                    $this->tableName = $table[$key]["cache"];
                    ...
                    $externID = (isset($this->inParams['static']['extern_id']) && ($this->inParams['static']['extern_id'] > 0)) ? $this->inParams['static']['extern_id'] : die(lgShowAlert("unknown cashID*. Please contact system administrator." . __LINE__));
...
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_kredit_periode"] = 0;
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $rekening_id, $extern2_id, $produk_id)
    {
        $tgl = date("d");
        $bln = date("m");
        $thn = date("Y");
