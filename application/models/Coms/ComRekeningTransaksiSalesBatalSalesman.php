...
class ComRekeningTransaksiSalesBatalSalesman extends MdlMother
{
...
//        "gudang_nama",
    ...
    {
        // TOTAL KONSOLIDASIAN
        $this->tableName = "z_sales_batal_salesman_cache";
        $this->tableName_master = array(
            "mutasi" => "z_sales_batal_salesman_mutasi",
        );
    }
                                    ...
                                    switch ($mode) {
                                        case "cache":
                                            foreach ($pSpec_mode as $sub_mode => $pSpec_mode_data) {
                                                ...
                                                arrPrintKuning($pSpec_mode_data);
//mati_disini();
                                                switch ($sub_mode) {
                                                    case "insert":
...
//    private function cekPreValue($rek, $cabang_id, $periode, $produk_id, $gudang_id, $date = NULL, $transaksi_id, $seller_id)
    private function cekPreValue($rek, $periode, $date = NULL, $transaksi_id, $seller_id)
    {
        if ($date != NULL) {
            $date_ex = explode("-", $date);
            $tgl = $date_ex[2];
