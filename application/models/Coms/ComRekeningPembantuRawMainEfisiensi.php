...
class ComRekeningPembantuRawMainEfisiensi extends MdlMother
{
...
//        matiHere();
        if (sizeof($this->inParams) > 0) {
            ...
            foreach ($this->periode as $periode) {
                ...
                $arrRekening = array();
                $array_params = $this->inParams;
//                foreach ($this->inParams as $array_params) {
//                    arrPrintPink($array_params['static']);
...
//                        $beda = array_diff($fieldsCache, $fieldsMutasi);
//                        arrPrint($beda);
...
//                    $tableNameMutasi,
                    $_preValues = $this->cekPreValueCache(
                        $tableNameCache,
                        $key,//rekening efisiensi
                        $array_params['static']['cabang_id'],
//                        $array_params['static']['toko_id'],
                        $periode,
                        $array_params['static']['extern_id'],//coa efisiensi
                            $array_params['static']['extern2_id'],//quality
                            $array_params['static']['extern3_id'],//wo
                            $array_params['static']['extern4_id'],//biaya
                            $array_params['static']['produk_id'],//project
...
//                            mati_disini(("insufficient balance for $key. <br>You requested $value. <br>Available: $preNumber. <br>Post-value should be: $afterNumber"));
//                        }
//                    }
                    ...
                    if ($pakai_cache == 1) {
                        switch ($afterPosition) {
                            ...
                            case "debet":
                                ...
                                break;
                            default:
                                mati_disini(__LINE__ . " gagal menentukan posisi rekening DEBET / KREDIT [$key] " . __FUNCTION__ . " on file " . __FILE__);
                                break;
                        }
...
//        $this->db->where(array("extern_id" => $externID));
    ...
    //--------------------------------------------------
    private function cekPreValueCache($tableName, $rek, $extern2_id,$extern3_id,$extern4_id,$cabang_id, $periode, $pihak_id, $fulldate,$produk_id)
    {
        if ($fulldate != NULL) {
            $fulldate_ex = explode("-", $fulldate);
            $tgl = $fulldate_ex[2];
...
//        $this->addFilter("extern3_id='$extern3_id'");
//        $this->addFilter("extern4_id='$extern4_id'");
//        $this->addFilter("produk='$produk_id'");
        $result = array();
        $localFilters = array();
        if (sizeof($this->filters) > 0) {
            foreach ($this->filters as $f) {
