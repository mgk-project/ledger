...
class ComRekeningPembantuEfisiensiBiayaWorkorderMain extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_efisiensi_wo_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_efisiensi_wo",
        );
...
//                    $this->tableName = $table[$key]["cache"];
                    ...
                    $externID = (isset($this->inParams['static']['extern_id'])) ? $this->inParams['static']['extern_id'] : die(lgShowAlert("unknown externID*. Please contact system administrator.".__LINE__));
                    $extern2ID = (isset($this->inParams['static']['extern2_id'])) ? $this->inParams['static']['extern2_id'] : die(lgShowAlert("unknown extern2ID*. Please contact system administrator.".__LINE__));
                    $extern3ID = (isset($this->inParams['static']['extern3_id'])) ? $this->inParams['static']['extern3_id'] : die(lgShowAlert("unknown extern3ID*. Please contact system administrator.".__LINE__));

                    //  region cek saldo awal
                    $_preValues = $this->cekPreValue($key, $this->inParams['static']['cabang_id'], $periode, $externID,$extern2ID,$extern3ID);
...
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_kredit_periode"] = 0;
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $rekening_id, $rekening2_id, $rekening3_id)
    {

        matiHere($rek."|cab |".$cabang_id."|periode |".$periode."|rek id |".$rekening_id."|rekening2 id|".$rekening2_id."|rekening 3 id|".$rekening3_id);
        $tgl = date("d");
        $bln = date("m");
        $thn = date("Y");
