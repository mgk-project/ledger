 ...
 * Time: 3:41 PM
 */
class ComLockerStockPlafonBankMutasiMain extends MdlMother
{
        ...
        if (sizeof($this->inParams) > 0) {
            ...
            if ($_preValue != null) {
                ...
                if ($pakaicache == 1) {
                    ...
                    cekKuning($this->outParams[$lCounter]["cache"]["debet"]);
                    if ($this->outParams[$lCounter]["cache"]["debet"] < 0) {
                        $msg = "Insufficient stock for " . $paramAsli['static']['extern_nama'] . " with state: " . $paramAsli['static']['state'] . ", needed: " . $paramAsli['static']['debet'] . ", avail: " . $_preValue;
//                            mati_disini($msg);
                        die(lgShowAlert($msg));
                    }
                    $where = array(
                ...
                if ($this->outParams[$lCounter]["mutasi"][$position_last] < 0) {
                    $msg = "Insufficient stock for " . $paramAsli['static']['nama'] . " with state: " . $paramAsli['static']['state'] . ", needed: " . $paramAsli['static']['debet'] . ", avail: " . $_preValue;
                    die(lgShowAlert($msg));
                }
                $tmpUpdateMutasi = array(
                    ...
                    "transaksi_jenis" => $paramAsli['static']['transaksi_jenis'],
...
//                    $tmpCache = $m->lookUpStockSumActive($paramAsli['static']['cabang_id'], $paramAsli['static']['gudang_id'], $paramAsli['static']['extern_id']);
    ...
    private function cekPreValue($cabang_id, $produk_id, $gudang_id)
    {
        ...
        cekHitam($this->db->last_query() . " :: " . sizeof($tmp));
        arrPrintWebs($tmp);
        if (sizeof($tmp) > 0) {
            $nilai = $tmp['debet'];
        }
