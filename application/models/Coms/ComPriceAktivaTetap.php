...
class ComPriceAktivaTetap extends MdlMother
{
    ...
    public function __construct()
    {
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_aktiva_tetap",
            "cache"  => "_rek_pembantu_aktiva_tetap_cache",
        );
    }
    ...
    {
        $this->load->model("Mdls/MdlHargaAktivaTetap");
        $l = new MdlHargaAktivaTetap();
                ...
                foreach ($pSpec as $sub_mode => $pSpec_mode_data) {
                    ...
                    switch ($sub_mode) {
                        case "insert":
                            ...
                            cekLime("$sub_mode :: " . $this->db->last_query());
                            break;
                        case "update":
                            $this->db->where('id', $id);
