...
class ComLockerValueDetail extends MdlMother
{
...
//                    if (base64_decode($paramAsli['static']['transaksi_id'], true) === true) {
            ...
            $exception = isset($paramAsli['static']['exception']) ? $paramAsli['static']['exception'] : 0;
            $produkID = isset($paramAsli['static']['produk_id']) ? $paramAsli['static']['produk_id'] : 0;
            $rejection = isset($paramAsli['static']['rejection']) ? $paramAsli['static']['rejection'] : 0;

            if ($exception == 1) {
                $paramAsli['static']['produk_id'] = $transaksiID;
                $paramAsli['static']['nama'] = $paramAsli['static']['transaksi_no'];
                $defaultTransID = 0;
                $this->outParams[$lCounter]['produk_id'] = $paramAsli['static']['produk_id'];
                $this->outParams[$lCounter]['nama'] = $paramAsli['static']['nama'];
                $this->outParams[$lCounter]['transaksi_id'] = 0;
            }
            ...
            if ($rejection == 1) {
                $produkID = isset($paramAsli['static']['referenceID']) ? $paramAsli['static']['referenceID'] : 0;
                $this->outParams[$lCounter]['produk_id'] = $produkID;
                unset($this->outParams[$lCounter]['nama']);
                $this->outParams[$lCounter]['transaksi_id'] = 0;
            }
            ...
            if ($_preValues != null) {
                $_preValue = $_preValues["nilai"];
                $_preValue_tbl_id = $_preValues["id"];
                $this->outParams[$lCounter]["nilai"] = ($paramAsli['static']['nilai'] + $_preValue);
                $this->outParams[$lCounter]["mode"] = "update";
                $this->outParams[$lCounter]["id_tbl"] = $_preValue_tbl_id;
            ...
            if ($pakai_exec == 1) {
                if (sizeof($this->outParams) > 0) {
                    ...
                    foreach ($this->outParams as $ctr => $params) {
                        ...
                        $this->load->model("Mdls/MdlLockerValueDetail");
                        $l = new MdlLockerValueDetail();
                        switch ($mode) {
                            case "new":
                                unset($params["mode"]);
