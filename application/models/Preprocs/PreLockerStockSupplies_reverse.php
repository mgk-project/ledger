 ...
 * Time: 11:09 AM
 */
class PreLockerStockSupplies_reverse extends CI_Model
{
...
//                $_preValue = $this->cekPreValue($paramAsli['static']['jenis'], $paramAsli['static']['cabang_id'], $paramAsli['static']['produk_id'], $paramAsli['static']['state'], $defaultOlehID, $defaultTransID, $defaultGudangID);
                $_preValue = $this->cekPreValue("supplies", $paramAsli['static']['cabang_id'], $paramAsli['static']['extern_id'], "active", 0, 0, $defaultGudangID);
                if ($_preValue != null) {
                    ...
                    if ($this->outParams[$lCounter]["jumlah"] < 0) {
                        ...
                        $msg = "Insufficient stock for " . html_escape($paramAsli['static']['nama']) . " with state: " . $paramAsli['static']['state'] . ", needed: " . $paramAsli['static']['produk_qty'] . ", avail: " . $_preValue;
                        die(lgShowAlert($msg));
                    }
                }
                else {
                    if ($this->outParams[$lCounter]["produk_qty"] < 0) {
...
//                }
            ...
            foreach ($this->inParams as $lCtr => $paramAsli) {
                ...
                foreach ($paramAsli['static'] as $key => $value) {
                    ...
                    $this->outParamss[$lCounterr]["produk_id"] = $paramAsli['static']['extern_id'];
                }
                $defaultOlehID = isset($paramAsli['static']['oleh_id']) ? $paramAsli['static']['oleh_id'] : 0;
                $defaultTransID = isset($paramAsli['static']['transaksi_id']) ? $paramAsli['static']['transaksi_id'] : 0;
                $defaultGudangID = $paramAsli['static']['gudang_id'];
...
//                $_preValue = $this->cekPreValue($paramAsli['static']['jenis'], $paramAsli['static']['cabang_id'], $paramAsli['static']['produk_id'], $paramAsli['static']['state'], $defaultOlehID, $defaultTransID, $defaultGudangID);
                $_preValue = $this->cekPreValue("supplies", $paramAsli['static']['cabang_id'], $paramAsli['static']['extern_id'], "hold", 0, 0, $defaultGudangID);
                if ($_preValue != null) {
                    ...
                    if ($this->outParamss[$lCounterr]["jumlah"] < 0) {
                        ...
                        $msg = "Insufficient stock for " . html_escape($paramAsli['static']['nama']) . " with state: " . "hold" . ", needed: " . $paramAsli['static']['produk_qty'] . ", avail: " . $_preValue;
                        die(lgShowAlert($msg));
                    }
                }
                else {
                    if ($this->outParamss[$lCounterr]["produk_qty"] < 0) {

                        $msg = "transaksi gagal, karena " . html_escape($paramAsli['static']['nama']) . " state " . "hold" . ", needed: " . $paramAsli['static']['produk_qty'] . ", avail: 0";
                        die(lgShowAlert($msg));
                    }
