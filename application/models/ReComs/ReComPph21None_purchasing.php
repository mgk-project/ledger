...
class ReComPph21None_purchasing extends MdlMother
{
    private $jenisTr;
...
//            arrPrint($spTmp);
            if(sizeof($spTmp) > 0){
                if(strlen($spTmp[0]->npwp) == 0){
                    $msg = htmlspecialchars($_SESSION[$cCode]['main']['pihakName']) . " tidak memiliki NPWP. Silahkan memilih pilihan Non NPWP/ jika akan tetap dipilih silahkan lengkapi data vendor pada field npwp.";
                    die(lgShowAlert($msg));
                }
