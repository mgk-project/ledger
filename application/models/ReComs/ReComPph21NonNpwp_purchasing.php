...
class ReComPph21NonNpwp_purchasing extends MdlMother
{
    private $jenisTr;
...
//            arrPrint($spTmp);
            if(sizeof($spTmp) > 0){
                if(strlen($spTmp[0]->npwp) > 0){
                    $msg = htmlspecialchars($_SESSION[$cCode]['main']['pihakName']) . "  memiliki NPWP. Silahkan untuk memilih pilihan NPWP.";
                    die(lgShowAlert($msg));
                }
