...
class MdlProdukPerSerialNumber extends MdlMother
{
    protected $tableName = "produk_per_serialnumber";
    ...
    protected $filters = array(
        "produk_per_serialnumber.status='1'",
        "produk_per_serialnumber.trash='0'",
    );
    ...
    public function lookupJoinSearchProdukSerial_ori($key)
    {
...
//            "$tbl_1.produk_serial_number" => $key,// serial dari pabrik
    ...
    {
        /*
         * query custom sengaja untuk blokir kalau di key in huruf kecil/spasi doang
         */
        $tbl_1 = $this->tableName;
        $tbl_2 = "produk";
...
//            "$tbl_1.produk_serial_number" => $key,// serial dari pabrik
        ...
        // Membuat array untuk menyimpan kondisi-kondisi binary
        $binaryConditions = array();
        foreach ($condites2 as $field => $value) {
            $binaryConditions[] = "`$field` =  '$value'";
        }

        // Menggabungkan kondisi-kondisi binary ke dalam query dengan operator OR
        $binaryQuery = "(".implode(' OR BINARY '  , $binaryConditions).")";
        $sql = "SELECT * FROM `produk_per_serialnumber` RIGHT JOIN `produk` ON `produk_per_serialnumber`.`produk_id` = `produk`.`id` where produk_per_serialnumber.status='1' and produk_per_serialnumber.trash='0'and BINARY ".$binaryQuery;
