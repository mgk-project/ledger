...
class MdlPaymentSource extends MdlMother
{
...
//        "satuan",
    ...
    public function getHutangTertua($customer_id){
        $this->db->from($this->tableName);
        $this->db->where("sisa >", 100);
        $this->db->where("target_jenis", 749);

        if ($customer_id !== "" && $customer_id !== null) {
            // Jika ada customer_id → ambil data customer itu saja
            $this->db->where("extern_id", $customer_id);
            $this->db->order_by("dtime", "ASC"); // ambil yang paling tua
            $this->db->limit(1);

            $query = $this->db->get();
            return $query->row(); // hanya satu record
        } else {
            // Kalau tidak ada customer_id → ambil semua data
            $query = $this->db->get();
            return $query->result();
        }
    }
