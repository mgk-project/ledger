...
class MdlProdukWebs extends MdlMother
{
    ...
    public function lookupAllWebs()
    {
        ...
        if(!isset($_SESSION['webs']['produk_spec'])){
            $data['produk'] = json_decode($this->curl->simple_get($this->produk));
            $datas = array();
            foreach ($data['produk'] as $datum) {
                $spec[$datum->id] = $datum;
            }

            // $datas['produk'] = $spec;
            $datas = $_SESSION['webs']['produk_spec'] = $spec;
        }
        else{

            $datas = $_SESSION['webs']['produk_spec'];
        }
