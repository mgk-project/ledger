...
function cekUangMukaRelasiSo($tr, $stepNumber, $paramsFilter = array(), $gate)
{

    $cCode = "_TR_" . $tr;

    $ci =& get_instance();
    $ci->load->model("Coms/ComRekeningPembantuCustomerDetail");
    $cs = New ComRekeningPembantuCustomerDetail();
...
//            //-------
            ...
            $saldo_uang_muka = $tmpResult[0]->kredit;
        }
...
////        "total_invoice_netto" => ".0",
        "saldo_uang_muka_relasi_so" => $saldo_uang_muka,
    );
//    arrPrint($result);
//    mati_disini();
    if ($gate == "items") {
