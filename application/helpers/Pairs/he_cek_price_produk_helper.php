...
function cekPriceProduk($tr, $stepNumber, $paramsFilter = array(), $gate)
{
...
//    $rekening = "persediaan produk";
    ...
    $ci =& get_instance();
    $ci->load->model("Mdls/MdlHargaProduk");
    $cs = New MdlHargaProduk();
    if(sizeof($pIDs)>0){
...
function cekAveragePriceProduk($tr, $stepNumber, $paramsFilter = array(), $gate)
{
...
//            cekPink("$key=$realVal");
    ...
    if (sizeof($tmpResult) > 0) {
        foreach ($tmpResult as $eSpec) {
            $result[$eSpec->produk_id] = $eSpec->jml_nilai/$eSpec->jml;
        }
    }
//cekHere(":: pairing items ::");
//arrPrint($result);
//    mati_disini();
    return $result;
}