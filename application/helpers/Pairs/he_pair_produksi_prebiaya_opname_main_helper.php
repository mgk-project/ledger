...
function cekPairProduksiPreBiayaOpnameMain($tr, $stepNumber, $paramsFilter = array(), $gate)
{
    $cCode = "_TR_" . $tr;
    $configUi = loadConfigModulJenis_he_misc($tr, "coTransaksiUi");
    $configCore = loadConfigModulJenis_he_misc($tr, "coTransaksiCore");
    $source = isset($configUi['pairMakers'][$stepNumber]['preBiayaMain']['source']) ? $configUi['pairMakers'][$stepNumber]['preBiayaMain']['source'] : null;
    $key = isset($configUi['pairMakers'][$stepNumber]['preBiayaMain']['sourceKey']) ? $configUi['pairMakers'][$stepNumber]['preBiayaMain']['sourceKey'] : array();
    ...
    if ($production == true) {
        ...
        if ($source != null) {
            if (isset($_SESSION[$cCode][$source]) && sizeof($_SESSION[$cCode][$source]) > 0) {
                if(sizeof($key) > 0){
                    foreach ($key as $val){
                        $_SESSION[$cCode][$source][$val."_rev"] = isset($_SESSION[$cCode][$source][$val]) ? $_SESSION[$cCode][$source][$val] : "";
                    }
                }
//                foreach ($_SESSION[$cCode][$source] as $mKey => $mVal) {
//
//
//                }
            }
//            else{
//                cekMerah("tidak ada $source");
//            }
//
        }
    }
