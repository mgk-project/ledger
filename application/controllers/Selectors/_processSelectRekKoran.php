 ...
 * Time: 8:45 PM
 */
class _processSelectRekKoran extends CI_Controller
{
    ...
    public function select()
    {
        ...
        $selectorPairModel = isset($_SESSION[$cCode]['main']['pairMdlName']) ? $_SESSION[$cCode]['main']['pairMdlName'] : $this->config->item('heTransaksi_ui')[$this->jenisTr]['selectorPairModel'];
        $selectorPairField = isset($this->config->item('heTransaksi_ui')[$this->jenisTr]['selectorPairField']) ? $this->config->item('heTransaksi_ui')[$this->jenisTr]['selectorPairField'] : array();

        $this->load->model("Mdls/" . $selectorSrcModel);
        $this->load->model("Mdls/" . $selectorPairModel);
        $b = new $selectorSrcModel();
        $c = new $selectorPairModel();
//arrPrint($selectorPairField);
//matiHEre($selectorSrcModel." ".$selectorPairModel);
        ...
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                //pair model
                $c->addFilter("id=" . $row->folders . "");
                $tmpPair = $c->lookUpAll()->result();
...
//                    arrPrint($selectorPairField);
                    ...
                    if (sizeof($tmpPair) > 0) {
                        arrPrint($tmpPair);
                        $tmp2 = array();
                        foreach ($tmpPair as $tmpPair_0) {
                            foreach ($selectorPairField as $ky => $targetKey) {
                                cekHitam($ky);
                                $tmp[$targetKey] = makeValue($ky, $tmp2, $tmp2, $tmpPair_0->$ky);
                            }
                        }
            ...
            if (sizeof($_SESSION[$cCode]['items']) > 0) {
                ...
                $_SESSION[$cCode]['main']['pihakID'] = $_SESSION[$cCode]['items'][$id]['pair_pihak_id'];
                $_SESSION[$cCode]['main']['pihakName'] = $_SESSION[$cCode]['items'][$id]['pair_pihak_name'];
                $_SESSION[$cCode]['main']['pihakRelId'] = $_SESSION[$cCode]['items'][$id]['id'];
                $_SESSION[$cCode]['main']['pihakRelName'] = $_SESSION[$cCode]['items'][$id]['nama'];
