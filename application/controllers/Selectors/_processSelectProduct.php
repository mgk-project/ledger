 ...
 * Time: 8:45 PM
 */
class _processSelectProduct extends CI_Controller
{
                ...
                if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                    ...
                    if (sizeof($priceMainConfig) > 0) {
                        if (isset($priceMainConfig[$_SESSION[$cCode]['main']['pihakMainName']])) {
                            ...
                            $priceFilter = $priceMainConfig[$_SESSION[$cCode]['main']['pihakMainName']]['mdlFilter'];
...
//                    die();
        ...
        if (sizeof($arrTrID) > 0) {
            //tambahan untuk grab nomertop di purcashing dari request
            $this->load->model("MdlTransaksi");
            $t = new MdlTransaksi();
            $t->addFilter("id='" . $_GET['singleRefID'] . "'");
            $arp = $t->lookupMainTransaksi()->result();
            $_SESSION[$cCode]['main']['references'] = $arrTrID;
            $_SESSION[$cCode]['main']['references_num'] = $arp[0]->nomer_top;
            //end tambahan
...
//        }
    ...
    public function selectNoQty()
    {
        ...
        $id = $_GET['id'];
        $jml = isset($_GET['jml']) ? $_GET['jml'] : 1;
        $noview = isset($_GET['noview']) ? "noview=1&" : "";
...
//                        $_SESSION[$cCode]['items'][$id]['jml'] = $_GET['newQty'];
        ...
        echo "<script>";
        echo "top.document.getElementById('result').src='" . base_url() . "ValueGate/buildValues/" . $this->jenisTr . "?" . $noview . "selID=$id';";
        echo "</script>";
    }
