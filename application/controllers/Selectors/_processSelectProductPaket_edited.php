...
//                                arrPrint($tmpD);
                                $tmpItems = array(
                                    ...
                                    "nett1" => isset($tmpD[$priceConfigItems['mainSrc']]) ? $tmpD[$priceConfigItems['mainSrc']] * $eSpec->jml : 0,
...
//                        $h->addFilter("jenis_value='" . $priceConfig['label'] . "'");
                    ...
                    if (sizeof($arrComponents) > 0) {
                        ...
                        foreach ($arrComponents[$id] as $iter => $iterArray) {
                            $subtotal_nett1 += $iterArray["nett1"];
                            arrPrint($iterArray);
...
//                            $tmpRslt[$iter]["disc_value"] = $tmp["harga"] -($subtotal_price_items2/$iterArray['jml']);
                            $tmpRslt[$iter]["disc_value"] = $dic_value;
                            $tmpRslt[$iter]["disc_persent"] = $dic_value / $iterArray['harga'] * 100;
                            ...
                            $tmpRslt[$iter]["produk_ord_hrg"] = $jual;
