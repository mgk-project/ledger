 ...
 * Time: 16:08
 */
class ProductEditor extends CI_Controller
{
    protected $koloms;
    ...
    public function edit_()
    {
        ...
        $this->load->model("Mdls/" . "MdlProdukWip");//untuk produk per fase yang terelasi dengan produk
        $this->load->model("Mdls/MdlHargaSupplies");

        $o = new MdlProdukRakitan();
        $pk = new MdlProdukKomposisi();
        $o2 = new MdlSupplies();
        $ob = new MdlProdukRakitanBiaya();
        ...
        $bRow = sizeof($biayas);
        $bDatas = array();
        foreach ($biayas as $bRows) {

            $bDatas[$bRows->produk_dasar_id] = $bRows->nilai;
        }

        // cekKuning("$dbRow != $bRow");
        $sync = 0;
        if ($dbRow != $bRow) {
            $sync = 1;
        }
        ...
        if (sizeof($components) > 0) {
            ...
            if ($sync == 1) {
                ...
                $url_sync = base_url() . "ProductEditor/syncKomposisiBiaya/$prodID";
        ...
        if (sizeof($_SESSION['PROED'][$prodID]['component']) > 0) {
            ...
            foreach ($_SESSION['PROED'][$prodID]['component'] as $id => $eSpec) {
                ...
                if (!isset($_SESSION['PROED'][$prodID]['component'][$id]['harga'])) {
                    $_SESSION['PROED'][$prodID]['component'][$id]['harga_old'] = $arrBahanPrice[$id];
                    $harga_bom = $_SESSION['PROED'][$prodID]['component'][$id]['harga'] = $arrBahanPrice[$id];
                    // cekHijau("buat");
                }
                else {
                    // cekMerah("baca");
                ...
                $content .= "<input type='number' tabindex='$cTab' name='harga[]' value='" . (isset($eSpec['harga']) && $eSpec['harga'] * 1 > 0 ? $eSpec['harga'] * 1 : 0) . "'
                    onblur =\"top.$('#result').load('" . base_url() . "_productEditor/addItem?sID=$prodID&bID=$id&harga='+encodeURI(this.value));\"
                    onchange =\"top.$('#result').load('" . base_url() . "_productEditor/addItem?sID=$prodID&bID=$id&harga='+encodeURI(this.value));\"
                ...
                $content .= "<input type='number' tabindex='$cTab' name='jml[]' onfocus='select()' value='" . $eSpec['jml'] . "'
                    onblur =\"top.$('#result').load('" . base_url() . "_productEditor/addItem?sID=$prodID&bID=$id&jml='+encodeURI(this.value));\"
                    onchange =\"top.$('#result').load('" . base_url() . "_productEditor/addItem?sID=$prodID&bID=$id&jml='+encodeURI(this.value));\"
                    class='form-control text-right'>";
                ...
                $content .= "<a class='text-red' href=# onClick =\"window.open_holdon();top.$('#result').load('" . base_url() . "_productEditor/removeItem/$prodID/" . $id . "');\"><span class='glyphicon glyphicon-remove'></span></a>";
                $content .= "</td>";
            ...
            $content .= "<tr>";
            $content .= "<td colspan='6' class='text-right text-bold text-uppercase'>Estimasi Bahan</td>";
            $content .= "<td style='vertical-align: middle;padding: 2px;margin: 0px;'>";
            ...
            $content .= "<caption class='text-bold text-grey font-size-1-2'>Standart Cost Production</caption>";
            $content .= "<tr>";
            ...
            if (isset($_SESSION['PROED'][$prodID]['cost'])) {
                ...
                foreach ($_SESSION['PROED'][$prodID]['cost'] as $costItem) {
                    ...
                    $content .= "<td class='text-right'>" . formatField('harga', $costItem['value']) . "</td>";
                    $content .= "</tr>";

                    $sumCost += $costItem['value'];
                }
                ...
                $content .= "<th class='text-right text-uppercase' colspan='2'>jumlah standat cost</th>";
                $content .= "<th class='text-right'>$sumCost_f</th>";
                $content .= "</tr>";

                $content .= "<tr class='bg-grey-1'>";
                $content .= "<th class='text-right text-uppercase' colspan='2'>total estimasi bom</th>";
            ...
            //             cekHitam(sizeof($_SESSION['PROED'][$prodID]['component']) ." vs  ". sizeof($components));
            if (sizeof($_SESSION['PROED'][$prodID]['component']) != sizeof($components)) {
                // $content .= "<tr class='text-center text-bold'>";
                $content .= "<div class='alert margin-top-10' style='background-color: yellow;'>";
    ...
    public function edit()
    {
        ...
        // $_SESSION['PROED'][$prodID]['gudang']["gudang_id"]=>$temproduk_0->gudang;
        ...
        $tempWip = $wp->lookUpAll()->result();
        if (sizeof($tempWip) > 0) {
            foreach ($tempWip as $ic => $tempWip_0) {
                $preTarget["produk_dasar_id"][$tempWip_0->id] = (array)$tempWip_0;
            }
        }
        ...
        if (sizeof($tempProdukFase) > 0) {
            foreach ($tempProdukFase as $data) {
                foreach ($fields as $field => $fieldAlias) {
                    $produkFase[$data->urut][$field] = isset($data->$field) ? $data->$field : "";
                }
                //numpang untuk rel next target fase wip
                $preTarget["fase_id"][$data->urut] = array(
                    "id"   => $data->urut,
                    "nama" => $data->nama,
                );
            }
        ...
        $data = array(
            ...
            "result"                          => $iframeTarget,
            "removeLink"                          => base_url() . get_class($this) . "/remove",
        );
        $this->load->view("editor", $data);
    }
...
//         matiHEre();
    ...
    public function remove()
    {
        // arrprint($_GET);
        $this->db->trans_start();
        $mastterID =$_GET["masterID"];
        $curentFase =$_GET["faseID"];
        $curentID =$_GET["id"];
        switch ($_GET["mode"]){
            case "komposisiProduk":
                $this->load->model("Mdls/MdlProdukKomposisiFase");
                $c = new MdlProdukKomposisiFase();
                $update =array(
                    "status"=>"0",
                    "trash"=>"1",
                    "trash_dtime"=>date("Y-m-d H:i"),
                    "trash_oleh_id"=>$this->session->login["id"],
                    "trash_oleh_nama"=>$this->session->login["nama"],
                );
                $c->setFilters(array());
                $update = $c->updateData(array("id"=>$curentID),$update) or matiHEre("gagal menon aktifkan komposisi errorcode : ".__LINE__);
                ceklime($this->db->last_query());
                $this->autoPatchSrcTargetWip($mastterID,$curentFase);
                // ceklime($this->db->last_query());
