 ...
 * Time: 3:28 PM
 */
class _productKompositEditor extends CI_Controller
{
...
//        $pp->addFilter("cabang_id='$selectedPlace'");
        $pp->setFilters(array());
        $pp->addFilter("produk_id='$id'");
        $pp->addFilter("status='1'");
        $pp->addFilter("trash='0'");
        $pp->addFilter("jenis_value in ('jual', 'hpp','jual_nppn')");
...
//            matiHere(__LINE__);
            ...
            $_SESSION['PROPKGED'][$prodID]['component'][$id] = array(
                ...
                "satuan" => $tmp[0]->satuan,
                "kode" => $tmp[0]->kode,
                "price" => isset($tmpPrice[$id]['jual_nppn']) ? $tmpPrice[$id]['jual_nppn']: $tmpPrice[$id]['jual']*1.11,
                "jual" => isset($tmpPrice[$id]['jual_nppn']) ? $tmpPrice[$id]['jual_nppn']: $tmpPrice[$id]['jual']*1.11,
                "harga" => isset($tmpPrice[$id]['jual_nppn']) ? $tmpPrice[$id]['jual_nppn']: $tmpPrice[$id]['jual']*1.11,
                "hpp" => $hpp,
                "jml" => 1,
                "subtotal" => isset($tmpPrice[$id]['jual_nppn']) ? $tmpPrice[$id]['jual_nppn']*1: ($tmpPrice[$id]['jual']*1.11)*1,
                "subharga" => isset($tmpPrice[$id]['jual_nppn']) ? $tmpPrice[$id]['jual_nppn']*1: ($tmpPrice[$id]['jual']*1.11)*1,
                "subhpp" => 1 * $hpp,
...
//            if(isset($_GET[""]))
            switch ($_GET["key"]){
                case "jml":
                case "harga":
//                    matiHere("__MASUK SINI");
                $_SESSION['PROPKGED'][$prodID]['component'][$id]['jml'] = $_GET['jml'];
                $_SESSION['PROPKGED'][$prodID]['component'][$id]['harga'] = $_GET['harga'];
                $_SESSION['PROPKGED'][$prodID]['component'][$id]['subtotal'] = $_GET['jml'] * $_GET['harga'];
//                $_SESSION['PROPKGED'][$prodID]['component'][$id]['subhpp'] = $_GET['jml'] * $_GET['hpp'];
                $_SESSION['PROPKGED'][$prodID]['component'][$id]['subharga'] = $_GET['jml'] * $_GET['harga'];
                    break;
                default:
                    break;
            }
...
//                $_SESSION['PROPKGED'][$prodID]['component'][$id]['subHarga'] = $_GET['jml'] * $tmpPrice[$id]['harga'];
//            }
        }
//arrPrint($_SESSION['PROPKGED'][$prodID]['component'][$id]);
//        matiHEre("debuging BID".$id);
...
//        $o->addFilter("trash='0'");
//        $o->addFilter("jenis in ('item')");
//        $o->addFilter("jenis in ('item', 'item_rakitan')");
...
//        else{
//            cekMerah("search tidak menemukan");
//        }
