 ...
 * Time: 3:28 PM
 */
class _productEditorPaket extends CI_Controller
{
...
//        $o = new MdlSupplies();
        ...
        if (!isset($_SESSION['PROED'][$prodID]['cost'])) {
            ...
            $pr = new MdlProdukRakitanBiayaPaket();
...
//                echo swalAlert($alert);
                ...
                echo "<script>
                        ...
                        top.swal({
                            title: '!! PERHATIAN !!',
                            html: 'BIAYA UNTUK PAKET INI BELUM DI TENTUKAN, KLIK OK UNTUK MELANJUTKAN KE SETTING BIAYA.',
                        })
                        .then((result) => {
                          if (result) {
...
//        $o = new MdlSupplies();
        $o = new MdlProduk2();
        $this->db->limit(50);
        $this->db->order_by("satuan_nilai", "ASC");
        $tmp = $o->lookupByKeyword($key)->result();
//        cekmerah($this->db->last_query());
        if (sizeof($tmp) > 0) {
            echo "<ul class='list-group'>";
            foreach ($tmp as $row) {
                if($row->kategori_id==3 && $row->sub_kategori_id==4){
                    //non unit tidak dikeluarkan
                }
                else{
                    if($row->kategori_id==1){
                        echo "<li kategori='".$row->kategori_id."' sub_kategori='".$row->sub_kategori_id."' style='padding: 3px;' class='list-group-item'>";
                echo "<a href=# onClick =\"top.$('#result').load('" . base_url() . get_class($this) . "/addItem?sID=$prodID&bID=" . $row->id . "$strDock');\">";
                        echo $row->nama . " <r>(" . $row->kategori_nama . " - " . $row->sub_kategori_nama . ")</r>";
                echo "</a>";
                        echo "</li>";
            }
                    else{
                        if($row->kategori_id==3 && $row->sub_kategori_id==5){
                            if($row->satuan_nilai*1==1){
                                echo "<li kategori='".$row->kategori_id."' sub_kategori='".$row->sub_kategori_id."' style='padding: 3px;' class='list-group-item'>";
                                echo "<a href=# onClick =\"top.$('#result').load('" . base_url() . get_class($this) . "/addItem?sID=$prodID&bID=" . $row->id . "$strDock');\">";
                                echo $row->nama . " <r>(" . $row->sub_kategori_nama . ")</r>";
                                echo "</a>";
                                echo "</li>";
        }
                            elseif($row->satuan_nilai*1==0){
                                echo "<li kategori='".$row->kategori_id."' sub_kategori='".$row->sub_kategori_id."' style='padding: 3px;' class='list-group-item'>";
                                echo "<a href=# onClick =\"top.$('#result').load('" . base_url() . get_class($this) . "/addItem?sID=$prodID&bID=" . $row->id . "$strDock');\">";
                                echo $row->nama . " <r>(" . $row->sub_kategori_nama . ")</r>";
                                echo "</a>";
...
//                                echo "<li kategori='".$row->kategori_id."' sub_kategori='".$row->sub_kategori_id."' style='padding: 3px;' class='list-group-item'>";
//                                echo "<a href=# onClick =\"top.$('#result').load('" . base_url() . get_class($this) . "/addItem?sID=$prodID&bID=" . $row->id . "$strDock');\">";
//                                echo $row->satuan_nilai . " - " . $row->nama . " <r>(" . $row->sub_kategori_nama . ")</r>";
//                                echo "</a>";
//                                echo "</li>";
//                            }
    }
...
//        cekmerah($this->db->last_query());
    ...
    public function extrackItem()
    {
        ...
        $dock = isset($_GET['dock']) ? $_GET['dock'] : "";
//        arrPrint($_GET);
        $this->load->model("Mdls/MdlProdukKomposisiPaket_and_cost");
        $o = new MdlProdukKomposisiPaket_and_cost();
